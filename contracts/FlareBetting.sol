...
contract FlareBetting {
    ...
    */
    ...
    modifier whileBetBeingResolved() {
        require(
            ...
            "The bet is being resolved"
        );
        _;
    }
    modifier afterBetHasBeenResolved() {
        require(
            (s_isBetCreated == true) && (_betStatus == BetStatus.BetEnded),
    ...
    */
    ...
    function startBet() external payable onlyOwner {
        if (
            msg.value <
            (convertUSDtoFLRinWei(i_maximumBetFromEitherTeamInUSD) /
                2 +
    ...
    ) external payable onlyAfterBetHasStarted whileBetIsBettingPeriodOngoing {
        require(
    ...
    function resolveBetWithFDC(
        uint256 difference
    ) external whileBetBeingResolved returns (bool isForSideWinner) {
        ...
        if (conditionMet) {
            ...
            if (s_againstBettersAddresses.length != 0) {
                ...
                s_randomRewardWinner = randomRewardWinner;
                s_againstBettersToAmountBetInUSD[
                    randomRewardWinner
                ] += s_randomRewardInUSD;
                emit randomWinnerPicked(randomRewardWinner);
            }
            for (uint256 i = 0; i < s_againstBettersAddresses.length; i++) {
                ...
                uint256 amountToBePaid = s_againstBettersToAmountBetInUSD[
                    winner
                ];
    ...
    */
    ...
    ) external payable afterBetHasBeenResolved {
        if (s_forBettersToAmountBetInUSD[user] != 0) {
            (bool sent, ) = user.call{
                value: convertUSDtoFLRinWei(s_forBettersToAmountBetInUSD[user])
            }("");
            require(sent, "Transfer failed");
        }
        if (s_againstBettersToAmountBetInUSD[user] != 0) {
            (bool sent, ) = user.call{
                value: convertUSDtoFLRinWei(
                    s_againstBettersToAmountBetInUSD[user]
                )
            }("");
            require(sent, "Transfer failed");
        }
    ...
    */
    function withdrawAmountOwner()
        external
        payable
        afterBetHasBeenResolved
        onlyOwner
    {
        uint256 ownersBalance = address(this).balance;
        for (uint256 i = 0; i < s_forBettersAddresses.length; i++) {
            address payable user = s_forBettersAddresses[i];
            ownersBalance -= convertUSDtoFLRinWei(
                s_forBettersToAmountBetInUSD[user]
            );
        }
        for (uint256 i = 0; i < s_againstBettersAddresses.length; i++) {
    ...
    */
    function changeBetStateToBetBeingResolved()
        external
        whileObservationPeriodOngoing
        onlyOwner
    {
        require(
            block.timestamp >=
            ...
            "Still in observation time"
        );
        _betStatus = BetStatus.BetBeingResolved;
    }

    function changeBetStateToObservationPeriodOngoing()
        external
        whileBetIsBettingPeriodOngoing
        onlyOwner
    {
        require(
            block.timestamp >= s_betCreationTimeStamp + i_betPlacingInterval,
            "Still in betting time"
        );
        _betStatus = BetStatus.ObservationPeriodOnGoing;
    }
    ...
    function getmaximumBetFromEitherTeamInUSD()
        external
        view
        returns (uint256)
    {
    ...
    function timeRemainingInSecondsTillResult()
        external
        view
    ...
    function getTimeLeftToPlaceBetInSeconds() external view returns (uint256) {
        return
            i_betPlacingInterval - (block.timestamp - s_betCreationTimeStamp);
    }
    ...
    function getIfBetPlacingIntervalIsOver()
        external
        view
        onlyAfterBetHasStarted
        returns (bool)
    ...
    function getAmountBettedForByAddress(
        address payable Address
    ) external view returns (uint256) {
        return s_forBettersToAmountBetInUSD[Address];
    }

    function getAmountBettedAgainstByAddress(
        address payable Address
    ) external view returns (uint256) {
        return s_againstBettersToAmountBetInUSD[Address];
    }

    function getAmountBettedForByAddress__(
        address payable Address
    ) external view returns (uint256) {
        return s_forBettersToAmountBetInUSD[Address];
    }

    function getAmountBettedAgainstByAddress__(
        address payable Address
    ) external view returns (uint256) {
        return s_againstBettersToAmountBetInUSD[Address];
    }
